...
- **Retention originally kept the newest 7, by count rather than age.** A count bounds disk usage; "older than N days" does not, if the database grows. The sweep only ever considers files matching a strict `rwnd-<compact ISO>.sql.gz` pattern, mirroring the defence-in-depth instinct behind `BACKUP_ID_RE` in `apps/api/src/backup/paths.ts`: the directory is a bind mount a human may also put files in, so the job must be structurally incapable of deleting anything it did not write.
  ...
  **Update, 2026-09-17: the daily tier now also deduplicates to one dump per UTC calendar day**, rather than keeping every dump within the window outright as the 2026-09-10 update above describes. Found the gap the hard way: both dev and prod had accumulated several same-day duplicates each (restarts, and the new manual "back up now" route) that this tier was retaining every one of, pruned by hand rather than by the retention policy itself. `selectDumpsToKeep` now buckets the daily tier the same way it already bucketed the weekly/monthly tiers, just keyed by calendar day (`createdAt.toISOString().slice(0, 10)`) instead of a week/month index.
