...
- [ ] **Automatic, scheduled backup of the entire database**: automates the whole-Postgres `pg_dump`/`psql` restore `docs/self-hosting.md` already documents as a manual step, distinct from the existing per-user Settings > Database backup feature (see TODO.md).
