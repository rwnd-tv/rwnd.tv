...
<tr>
<td align="center">
<picture><source media="(prefers-color-scheme: dark)" srcset="docs/screenshots/calendar-en-US-dark.webp"><img src="docs/screenshots/calendar-en-US-light.webp" alt="Calendar page: a forward-looking agenda of upcoming episodes, grouped by day"></picture>
<br><sub>Calendar, full size: <a href="docs/screenshots/calendar-en-US-light.webp">light</a> · <a href="docs/screenshots/calendar-en-US-dark.webp">dark</a></sub>
</td>
<td align="center">
<picture><source media="(prefers-color-scheme: dark)" srcset="docs/screenshots/history-en-US-dark.webp"><img src="docs/screenshots/history-en-US-light.webp" alt="History: every watch, rating, watchlist add and drop in one timeline"></picture>
<br><sub>History, full size: <a href="docs/screenshots/history-en-US-light.webp">light</a> · <a href="docs/screenshots/history-en-US-dark.webp">dark</a></sub>
</td>
</tr>
</table>
...
- **Calendar**: an in-app Calendar page over the same data the feeds serve, with a forward-looking Agenda list of what is still to come and a Month grid for a whole month at a glance, filterable by history, TV shows and films
- **Stats**: a `/stats` page covering totals, time watched, your top shows and movies, a year selector (doubling as a Year in Review recap), an activity-over-time chart, a day-of-week/hour-of-day heatmap, top genres, and a ratings histogram
- **Getting your data in and out**: Trakt import (OAuth device flow or a ZIP export upload, either works with no callback URL or reachable server), CSV import/export round-tripping the same open format, per-user backup/restore independent of a full database dump, automatic daily whole-database backups with an admin-editable daily/weekly/monthly retention policy, owner-only whole-database restore automation with an automatic pre-restore snapshot as a one-click undo point, and personal webcal/iCal calendar feeds (watch history, upcoming episodes, movie release dates) to subscribe to from Google/Apple/any other compatible calendar app
- **Automation**: Plex, Jellyfin, Emby and Tautulli webhook ingestion, each logging watches as they happen, with first-class multi-user support (each media-server account linked to its own rwnd.tv user); per-user API tokens back it
- **Instance**: light/dark/system theming, en-GB/en-US locales, a documented HTTP API (OpenAPI 3.1, session-gated Swagger UI at `/api/docs`), a configurable home page (rwnd.tv's own marketing page, or straight to sign-in, for a self-hosted instance that wants to skip it), and an admin page (user management: a searchable/filterable/sortable list plus a per-user detail page with roles including an owner role immune to demotion, last login, sessions, password reset, account deletion, and bulk actions across several accounts at once; instance settings covering name, registration mode (open/invite/closed), the public admin contact email, and metadata-provider priority order; invite codes to create, list, and revoke when registration is invite-only; plus automatic-backup status, the retained dumps, and the retention policy, editable without shell access)
