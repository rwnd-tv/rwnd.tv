...
| CodeQL alert #1 (recurred as alert #16 on 2026-09-14, after an unrelated edit shifted the line number): `hashSecret()`'s unsalted SHA-256 flagged as a weak password hash (`js/insufficient-password-hash`) | Dismissed as a false positive both times; now suppressed inline (`// codeql[js/insufficient-password-hash]`) so a future line shift can't reopen it a third time | `hashSecret()` only ever hashes 256-bit CSPRNG values (session/API/recovery/webhook tokens), never a password: see "Bearer secrets are hashed, not encrypted" above and the comment on `hashSecret()` itself (`apps/api/src/lib/tokens.ts`). CodeQL's static rule can't see that distinction.                                                    |
...
shows only that server's own setup steps, and each existing webhook is
its own collapsible panel instead of a shared list.
...
`packages/shared/src/schemas/settings.ts`), and a token without one
falls back to the pre-2026-09-14 behavior for that one token: still
...
This changes what a database compromise costs for this token class: previously,
...
cost from one-time reveal was real and recurring, weighed against a
