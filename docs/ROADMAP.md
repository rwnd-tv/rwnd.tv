...
- [ ] **Trakt ZIP-upload import**: a second Trakt import path from the ZIP "Export now" gives on trakt.tv, alongside the existing OAuth-based import — sidesteps Trakt's 2026 "Community App" policy, which caps free accounts at one connected third-party OAuth app at a time (see TODO.md)
...
- [ ] **"Forgot password" / account recovery**: a self-service password-reset flow for local accounts, needing outbound email and a reset-token flow (see TODO.md)
