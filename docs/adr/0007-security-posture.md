...
| No `__Host-` cookie prefix                                                                                                                                   | Accepted for now                                                     | `__Host-` mandates `Secure`, which would break a plain-HTTP LAN-only deployment (a legitimate, documented configuration — `docs/self-hosting.md`). Worth adopting conditionally on `COOKIE_SECURE` being true; not done in this review to keep Stage D's header changes to what's unconditionally safe.                                           |
