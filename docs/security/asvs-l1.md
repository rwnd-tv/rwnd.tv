...
| V1.12.2        | Fail → fixed Stage D/F | Avatar served with a client-supplied `Content-Type` and no `Content-Disposition` (the `PUT /auth/me/avatar` handler in `apps/api/src/routes/auth.ts`) |
...
| V4.3.1 (admin MFA) | Fail → fixed         | Opt-in TOTP MFA for any user (`apps/api/src/lib/totp.ts`, `apps/api/src/routes/mfa.ts`), per RFC 6238, verified against the RFC's own published test vectors; single-use recovery codes; a confirmed-TOTP account gets a short-lived challenge from `POST /auth/login` instead of a session, exchanged at `POST /auth/login/mfa`. Requires `ENCRYPTION_KEY` (same AES-256-GCM path as Trakt tokens); hides itself in the UI when unset, same as any other optional-credential feature |
...
| V3.2.1 | Pass                     | `createSession()` mints a fresh token on every login: `apps/api/src/lib/session.ts`                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| V3.4.2 | Pass                     | `httpOnly: true`: `apps/api/src/lib/cookies.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| V3.4.3 | Pass                     | `sameSite: 'Lax'`: `apps/api/src/lib/cookies.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| V3.4.4 | Fail → fixed             | `__Host-` prefix applied whenever `COOKIE_SECURE` is true, in `apps/api/src/lib/cookies.ts`'s `sessionCookieName()`. Left plain on a self-hoster's LAN-only, plain-HTTP deployment, where the prefix's mandatory `Secure` would make the browser drop the cookie entirely                                                                                                                                                                                                                 |
...
| V4.2.1 (IDOR) | Pass                 | Cross-user isolation extensively tested. `assignableUsers`/cross-user webhook attribution (F-14/F-15) was a documented intentional multi-user-Plex design, not IDOR; the underlying consent gap it left open was closed by the link-code rework (2026-09-02, see `docs/adr/0007-security-posture.md`'s addendum) |
| V4.2.2 (CSRF) | Fail → fixed Stage C | No CSRF defence at all; 6 multipart routes reachable with a session cookie (F-05)                                                                                                                                                                                                                                |
| V4.3.1        | Fail → fixed         | See V2 table: TOTP MFA now available (opt-in, any user)                                                                                                                                                                                                                                                          |
...
| Req    | Status               | Evidence / rationale                                                                                                                                                                                                                                       |
| ------ | -------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| V7.1.1 | Fail → fixed Stage G | No credentials/tokens logged; user email addresses used to appear in failure logs in `apps/api/src/routes/auth.ts`, fixed to log `user.id` instead (F-17); `TMDB_API_KEY` reachable via a stringified request URL (`apps/api/src/providers/tmdb.ts`, F-09) |
| V7.4.1 | Pass                 | `app.onError()` returns a generic `{error: 'Internal Server Error'}`: `apps/api/src/app.ts`                                                                                                                                                                |
...
| V8.3.1 | Accepted risk (webhook), Fail → fixed Stage G (TMDB) | Plex webhook token is a URL path segment by necessity (Plex allows no custom headers); documented rationale in the doc comment on `webhookRoutes` in `apps/api/src/routes/webhooks.ts`; `TMDB_API_KEY` in the outbound query string is unrelated and gets fixed (F-09) |
...
| V14.3.2 | Pass                   | `NODE_ENV=production` set in `Dockerfile`; no debug endpoints found                                                                                                                                                                                                                                                                                                                                                           |
| V14.3.3 | Pass                   | Swagger UI's `info.version` now reflects the real running version (`APP_VERSION`, `apps/api/src/version.ts`) rather than a stale literal; not a version leak either way, since `GET /settings` already exposes it unauthenticated (`appVersion`, used by the landing page and Settings → About) and `/api/docs`/`/openapi.json` are gated behind a session anyway (Stage B, F-10); no other version/build info exposure found |
...
emails; see the V2.5.5 row above), F-27 / V2.1.7 (breached-password
check, HIBP k-anonymity; see the V2.1.7 row above), and V4.3.1 (opt-in TOTP
MFA; see the V4.3.1 row above). Image signing (cosign/
OIDC keyless) and
