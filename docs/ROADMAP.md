...
- [x] **Automatic, scheduled backup of the entire database**: a daily in-process `pg_dump`, gzipped, keeping the newest 7, gated on `DATABASE_BACKUP_DIR`. Automates what `docs/self-hosting.md` documented as a manual step, and stays distinct from the per-user Settings > Database feature. An older backup restores onto a newer version, since the dump carries its schema and the app's migrations carry it forward. See [ADR 0008](adr/0008-database-backups.md) and TODO_ARCHIVE.md.
