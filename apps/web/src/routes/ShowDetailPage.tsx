...
export function ShowDetailPage() {
  ...
  // SeasonDetailPage.tsx's own History table — each season's own
  // watchedEpisodes is specials-included (unlike the show-level one above),
  // so this correctly stays gated open for a show whose only watch is a
  // special.
  const showHasWatches = show?.seasons.some((season) => season.watchedEpisodes > 0) ?? false

  const { data: showWatchesData } = useQuery({
    queryKey: ['show', slug, 'watches'],
    queryFn: () => api.library.showWatches(slug!),
