...
- [ ] **Bulk select/actions on the admin Users list**: mass delete, password reset, and session revoke at minimum (possibly promote/demote too), building on the summary-row shape the per-user detail page split left `UsersPanel.tsx` with (see TODO.md).
