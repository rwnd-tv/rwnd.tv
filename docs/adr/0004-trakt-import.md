...
- **Resolving episode metadata at import scale.** `resolveEpisode()` (`apps/api/src/lib/media.ts`) fetches one episode at a time from TMDB, fine for logging a single watch but not for a history that can contain thousands of episodes across a handful of shows.
...
- **Unmatched items are reported, not dropped.** Anything without a resolvable TMDB id, or a Trakt item type rwnd.tv has no local entity for (`season`-level ratings/watchlist entries: `metadata_entity_type` has no `season` value), is recorded in the job's `failures` array, every one of them, uncapped, rather than silently skipped or failing the whole job.
